        ...
        if($query){
            ...
            if(count($words)>1){
                foreach ($words as $key => $word) {
                    $queryString=array();
                    $queryString[]="CONCAT(COALESCE(e.name,''),COALESCE(e.description,'')) LIKE :word".$key;
                    $qb->setParameter('word'.$key,"%".$word."%");
                    $qb->andWhere(join(' AND ',$queryString));
                }
            }else{
                $qb->andWhere("CONCAT(COALESCE(e.name,''),COALESCE(e.description,'')) LIKE :query")->setParameter('query',"%".$query."%");
            }
        }
        return $qb;
    }
